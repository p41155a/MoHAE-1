...
class MapViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    var slideWidth : CGFloat = 50
    var menuLeftConstraints: NSLayoutConstraint?
    var menuSlideWidthConstraints: NSLayoutConstraint?
...
extension MapViewController {
    func setupMapView(){
        ...
        guard  let navbarHeight = self.navigationController?.navigationBar.frame.maxY else {return}
            menuSlideWidthConstraints = listView.widthAnchor.constraint(equalToConstant: slideWidth)
            menuSlideWidthConstraints!.isActive = true
            listView.topAnchor.constraint(equalTo: self.view.topAnchor, constant: navbarHeight).isActive = true
